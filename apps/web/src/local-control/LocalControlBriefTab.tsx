...
function describeLocalControlError(error: unknown, fallback: string) {
  if (error instanceof LocalControlError && error.message) return error.message;
  if (error instanceof Error && error.message) return error.message;
  return fallback;
}

function LocalBriefItemCard({
  item,
  note,
  byline,
  hero = false,
  ...
  const resolvedByline = byline?.trim() || item.authors.join(", ").trim() || item.source_name;
  const publishedLabel = item.published_at ? formatDisplayDate(item.published_at.slice(0, 10)) : "Undated";
  ...
  return (
    <article
      className={
        hero
          ? "editorial-panel bg-[rgba(255,255,255,0.62)] shadow-[0_30px_80px_rgba(17,19,18,0.14)]"
          : "editorial-panel h-full"
      }
    >
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-3">
          ...
          <h3 className={hero ? "font-display text-4xl leading-tight text-[var(--ink)]" : "font-display text-2xl leading-tight text-[var(--ink)]"}>
            <Link className="hover:text-[var(--accent)]" to={`/documents/${item.id}`}>
              {item.title}
            </Link>
          </h3>
        </div>
        <a
          ...
          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[var(--ink)]/10 bg-[rgba(255,255,255,0.78)] text-[var(--muted)] transition hover:-translate-y-0.5 hover:border-[var(--accent)]/26 hover:text-[var(--accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/18"
          href={canonicalUrl}
          rel="noreferrer"
          target="_blank"
          title="Open source"
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <Link className="secondary-button" to={`/documents/${item.id}`}>
          Open detail
        </Link>
        <a className="secondary-button" href={canonicalUrl} rel="noreferrer" target="_blank">
          Open source
        </a>
      </div>
    </article>
...
function Headlines({ items }: { items: DigestEntry[] }) {
  ...
  const expandLabel = maxExpandedCount >= 20 ? "Expand to top 20" : `Expand to ${maxExpandedCount} reads`;
    ...
    <section className="editorial-panel">
      ...
      <div className="mt-6 overflow-hidden rounded-[1.75rem] border border-[var(--ink)]/8 bg-[rgba(255,255,255,0.48)]">
        {visibleItems.map((entry, index) => {
          ...
          return (
            <article
              className="group grid gap-3 border-b border-[var(--ink)]/8 px-5 py-4 transition last:border-b-0 hover:bg-[rgba(255,255,255,0.36)] md:grid-cols-[minmax(0,3fr)_minmax(220px,1fr)] md:items-start md:gap-6 md:px-6"
              key={entry.item.id}
            >
              <Link className="min-w-0" to={`/documents/${entry.item.id}`}>
              ...
              <div className="flex items-center justify-between gap-4 border-t border-[var(--ink)]/8 pt-3 md:border-t-0 md:pt-0">
                ...
                <div className="flex items-center gap-2">
                  <Link className="secondary-button" to={`/documents/${entry.item.id}`}>
                    Detail
                  </Link>
                  <a
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <SkimmableText className="mt-3 max-w-4xl text-base leading-7 text-[var(--muted)]">
            The strongest research papers in this window, annotated with best-effort Zotero tags and a credibility score based on lab prestige, citations, and source quality.
          </SkimmableText>
        </div>
        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[var(--ink)]/10 bg-[rgba(255,255,255,0.56)] px-4 py-2 font-mono text-[11px] uppercase tracking-[0.22em] text-[var(--muted)]">
          <span>{String(items.length).padStart(2, "0")}</span>
          ...
          >
            <div className="min-w-0">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <Link className="text-lg font-semibold leading-7 text-[var(--ink)] hover:text-[var(--accent)]" to={`/documents/${entry.item.id}`}>
                    {entry.item.title}
                  </Link>
...
function AudioBriefSummaryStrip({ brief }: { brief: Digest }) {
  ...
  const audioRequestInit = useMemo<RequestInit | undefined>(
    () => (token ? { headers: { Authorization: `Bearer ${token}` } } : undefined),
    [token],
  );
  const generateAudio = useMutation({
    mutationFn: () => localControlClient.runGenerateAudio(brief.brief_date ?? ""),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["local-control", "briefs"] });
    },
  });

  if (!brief.brief_date) return null;

  const isBuilding =
    audioBrief?.status === "running" || audioBrief?.status === "pending" || generateAudio.isPending;
  const notice =
    generateAudio.isError
      ? describeLocalControlError(generateAudio.error, "The voice summary could not be generated.")
      ...
      {audioBrief?.status === "succeeded" ? (
        <>
          <AudioBriefPlayer
            ...
            audioRequestInit={audioRequestInit}
            audioUrl={localControlClient.getBriefAudioUrl(brief.brief_date)}
...
export function LocalControlBriefTab() {
  ...
  const brief = useQuery({
    queryKey: ["local-control", "briefs", selectedPeriod?.periodType, selectedPeriod?.value],
    queryFn: () =>
      selectedPeriod?.periodType === "week"
        ? localControlClient.getWeekBrief(selectedPeriod.value)
        : localControlClient.getBrief(selectedPeriod?.value ?? ""),
  ...
  if (availability.error || !availability.data) {
    return (
      <div className="editorial-panel">
        <p className="section-kicker">Brief unavailable</p>
        <p className="mt-3 text-sm leading-6 text-[var(--danger)]">
          {describeLocalControlError(availability.error, "Brief availability is not available right now.")}
        </p>
      </div>
    );
  }

  if (!availability.data.days.length && !availability.data.weeks.length) {
    return (
      <div className="editorial-panel">
        <p className="section-kicker">No editions yet</p>
        <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
          No brief windows are available yet on this Mac. Open the pipeline tab and regenerate the current brief first.
        </p>
        <Link className="primary-button mt-6 w-fit" to="/pipeline">
          Open pipeline
        </Link>
      </div>
    );
  ...
  if (brief.isLoading) {
    return <div className="page-loading">Loading the selected brief…</div>;
  }

  if (brief.error || !brief.data) {
    return (
      <div className="editorial-panel">
        <p className="section-kicker">Edition unavailable</p>
        <p className="mt-3 text-sm leading-6 text-[var(--danger)]">
          {describeLocalControlError(brief.error, "That brief window is not available yet.")}
        </p>
        <Link className="secondary-button mt-6 w-fit" to="/pipeline">
          Open pipeline
        </Link>
      </div>
    );
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <SkimmableText className="mt-4 max-w-4xl text-sm leading-6 text-[var(--muted)]">
                This is the brief the Mac generated from the local vault. Use the document detail pages for raw text, filed notes, and source artifacts when you want to drill down.
              </SkimmableText>
            </div>
          ...
          <section className="editorial-panel-dark">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {brief.data.suggested_follow_ups.map((question, index) => (
                <article className="rounded-[1.45rem] border border-white/10 bg-white/6 p-5" key={question}>

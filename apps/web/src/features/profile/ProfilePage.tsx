...
type AlphaXivSourceOption = NonNullable<Profile["alphaxiv_search_settings"]["source"]> | "all";
type AlphaXivSearchForm = {
  topics: string;
  organizations: string;
  sort: Profile["alphaxiv_search_settings"]["sort"];
  interval: Profile["alphaxiv_search_settings"]["interval"];
  source: AlphaXivSourceOption;
};
...
const DEFAULT_ALPHAXIV_SEARCH: AlphaXivSearchForm = {
  topics: "",
  organizations: "",
  sort: "Hot",
  interval: "30 Days",
...
const ALPHAXIV_SORT_OPTIONS: Profile["alphaxiv_search_settings"]["sort"][] = [
  "Hot",
  "Comments",
  "Views",
  "Likes",
...
const ALPHAXIV_INTERVAL_OPTIONS: Profile["alphaxiv_search_settings"]["interval"][] = [
  "3 Days",
  "7 Days",
  "30 Days",
  "90 Days",
...
const ALPHAXIV_SOURCE_OPTIONS: Array<{ value: AlphaXivSourceOption; label: string }> = [
...
export function ProfilePage() {
  ...
  const [alphaxivSearch, setAlphaxivSearch] = useState<AlphaXivSearchForm>(DEFAULT_ALPHAXIV_SEARCH);
  ...
  const syncFormFromProfile = (profile: Profile) => {
    ...
    setAlphaxivSearch({
      topics: listToCsv(profile.alphaxiv_search_settings.topics),
      organizations: listToCsv(profile.alphaxiv_search_settings.organizations),
      sort: profile.alphaxiv_search_settings.sort,
      interval: profile.alphaxiv_search_settings.interval,
      source: profile.alphaxiv_search_settings.source ?? "all",
    });
  };
  ...
  const alphaxivTopics = csvToList(alphaxivSearch.topics);
  const alphaxivOrganizations = csvToList(alphaxivSearch.organizations);
  ...
  const onSubmit = (event: FormEvent) => {
    ...
    saveProfile.mutate({
      ...
      alphaxiv_search_settings: {
        topics: alphaxivTopics,
        organizations: alphaxivOrganizations,
        sort: alphaxivSearch.sort,
        interval: alphaxivSearch.interval,
        source: alphaxivSearch.source === "all" ? null : alphaxivSearch.source,
      },
    });
  };
    ...
    <div className="space-y-6 pb-10">
      <section className="editorial-panel">
        ...
        <SkimmableText className="mt-4 max-w-3xl text-base leading-7 text-[var(--muted)]">
          The profile still saves as one record, but the system reads it from several different pipelines. The controls below are grouped by where they are consumed so it is clear whether you are changing visibility, ranking, brief composition, audio behavior, or prompt guidance.
        </SkimmableText>
      </section>
      ...
      <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_340px]">
        <form className="min-w-0 space-y-6" onSubmit={onSubmit}>
          ...
          <PipelineCard
            description="This controls how the alphaXiv paper source queries the site feed before the per-run document cap is applied. Use it to widen the time window, bias the ranking mode, or narrow discovery to specific topics or organizations."
            kicker="Pipeline 08"
            title="alphaXiv discovery"
            usedBy={["alphaXiv fetch", "Manual source inject", "Source probe"]}
          >
            <div className="grid gap-4 lg:grid-cols-3">
              <label>
                ...
                <select
                  className="field-input"
                  value={alphaxivSearch.interval}
                  onChange={(event) =>
                    setAlphaxivSearch({
                      ...alphaxivSearch,
                      interval: event.target.value as Profile["alphaxiv_search_settings"]["interval"],
                    })
                  }
                >
                  {ALPHAXIV_INTERVAL_OPTIONS.map((value) => (
                    <option key={value} value={value}>
                      {value}
                    </option>
                  ))}
                </select>
                <FieldHint>Choose the feed window first. If you want a three-month backfill, set this to 90 Days and raise the injection max-items cap.</FieldHint>
              </label>
              <label>
                <span className="field-label">Sort</span>
                <select
                  className="field-input"
                  value={alphaxivSearch.sort}
                  onChange={(event) =>
                    setAlphaxivSearch({
                      ...alphaxivSearch,
                      sort: event.target.value as Profile["alphaxiv_search_settings"]["sort"],
                    })
                  }
                >
                  {ALPHAXIV_SORT_OPTIONS.map((value) => (
                    <option key={value} value={value}>
                      {value}
                    </option>
                  ))}
                </select>
                <FieldHint>Changes how alphaXiv ranks the pool before the ingester trims it down to the requested document count.</FieldHint>
              </label>
              <label>
                <span className="field-label">Source filter</span>
                <select
                  className="field-input"
                  value={alphaxivSearch.source}
                  onChange={(event) =>
                    setAlphaxivSearch({
                      ...alphaxivSearch,
                      source: event.target.value as AlphaXivSourceOption,
                    })
                  }
                >
                  {ALPHAXIV_SOURCE_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
                </select>
                <FieldHint>Leave this on All sources for the default feed, or lock it to GitHub or Twitter (X) when you want social-signal-specific papers.</FieldHint>
              </label>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label>
                ...
                <textarea
                  ...
                  placeholder="agents, reasoning, retrieval, multimodal"
                  value={alphaxivSearch.topics}
                />
                <FieldHint>Comma-separated alphaXiv topics. When blank, the feed stays broad and only interval or sort settings apply.</FieldHint>
              </label>
              <label>
                <span className="field-label">Organizations</span>
                <textarea
                  className="field-input min-h-28"
                  onChange={(event) =>
                    setAlphaxivSearch({
                  ...
                  value={alphaxivSearch.organizations}
                />
                <FieldHint>Comma-separated organization filters. Keep this sparse because alphaXiv expects exact organization labels.</FieldHint>
              </label>
            </div>
          </PipelineCard>
          ...
          <section className="editorial-panel space-y-4">
            <div>
              ...
              <SkimmableText className="mt-4 max-w-3xl text-sm leading-7 text-[var(--muted)]">
                All eight sections write back to the same shared profile record so the downstream pipelines stay aligned while still being configured separately.
              </SkimmableText>
            </div>
            {saveProfile.isError ? (
        ...
        <aside className="min-w-0 space-y-4">
          <section className="editorial-panel-dark">
            ...
            <h4 className="mt-3 font-display text-3xl leading-tight">How the system is wired right now</h4>
            <div className="mt-5 space-y-4">
              ...
              <div className="rounded-[1.5rem] border border-white/10 bg-white/6 p-4">
                <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-[rgba(236,228,211,0.68)]">alphaXiv feed</p>
                <p className="mt-3 text-sm leading-6 text-[rgba(236,228,211,0.92)]">
                  {alphaxivSearch.interval} window, {alphaxivSearch.sort} sort, {alphaxivSearch.source === "all" ? "all sources" : alphaxivSearch.source}
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="editorial-panel">
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <p className="text-sm leading-6 text-[var(--muted)]">Filters which stored records the app is allowed to show before any downstream logic runs.</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-sm font-semibold leading-6 text-[var(--ink)]">alphaXiv discovery</p>
                <p className="text-sm leading-6 text-[var(--muted)]">Shapes the alphaXiv source query before manual fetch max-items applies, so interval and filter choices decide which papers are even eligible for ingest.</p>
              </div>
            </div>
          </section>
          ...
          <section className="editorial-panel">
            ...
            <div className="mt-4 space-y-4 text-sm leading-6 text-[var(--muted)]">
              ...
              <div>
                <p className="font-semibold text-[var(--ink)]">alphaXiv filters</p>
                <p>
                  {alphaxivTopics.length || alphaxivOrganizations.length
                    ? `${alphaxivSearch.interval} / ${alphaxivSearch.sort} with ${alphaxivTopics.length} topic filter${alphaxivTopics.length === 1 ? "" : "s"} and ${alphaxivOrganizations.length} organization filter${alphaxivOrganizations.length === 1 ? "" : "s"}.`
                    : `${alphaxivSearch.interval} / ${alphaxivSearch.sort} with no topic or organization filters.`}
                </p>
              </div>
            </div>
          </section>
